...
CREATE TABLE `iblue_fdm`.`streets` (
  ...
  `decimal_latitude_1` decimal(10,6) NOT NULL,
  `decimal_longitude_1` decimal(10,6) NOT NULL,  
  ...
  `decimal_latitude_2` decimal(10,6) NOT NULL,
  `decimal_longitude_2` decimal(10,6) NOT NULL, 
...
CREATE INDEX street_lat_long_1 ON `iblue_fdm`.`streets`(decimal_latitude_1, decimal_longitude_1);
...
WHERE 
	st.decimal_latitude_1 BETWEEN lat - 0.0015 AND lat + 0.0015
    AND st.decimal_longitude_1 BETWEEN lon - 0.0015 AND lon + 0.0015
    AND st.int_status = 1
ORDER BY dist ASC
LIMIT 1;
END $$
...
SELECT
	st.pk_id
	,st.decimal_latitude_1
    ,st.decimal_longitude_1
	,st.decimal_latitude_2
    ,st.decimal_longitude_2
...
WHERE 
	st.decimal_latitude_1 BETWEEN lat - 0.05 AND lat + 0.05
    AND st.decimal_longitude_1 BETWEEN lon - 0.05 AND lon + 0.05
    AND st.int_status = 1;
END $$
DELIMITER ;
...
SELECT
    ...
    ,ABS(st.decimal_latitude_1 - lat) + ABS(st.decimal_longitude_1 - lon) as dist_app
FROM `iblue_fdm`.streets st
	LEFT JOIN `iblue_fdm`.vw_in_use_spots usp
		ON st.pk_id = usp.fk_street_id
WHERE 
	st.decimal_latitude_1 BETWEEN lat - 0.015 AND lat + 0.015
    AND st.decimal_longitude_1 BETWEEN lon - 0.015 AND lon + 0.015
    AND st.int_status = 1
