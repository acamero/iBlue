...
public class StandardPSO2011 extends AbstractParticleSwarmOptimization<DoubleSolution, DoubleSolution> {
	...
	private double c;
	private ExtendedPseudoRandomGenerator randomGenerator;
	 ...
	 * @param problem
	 * @param objectiveId
	 *            This field indicates which objective, in the case of a
	 *            multi-objective problem, is selected to be optimized.
	 * @param swarmSize
	 * @param maxIterations
	...
	public StandardPSO2011(DoubleProblem problem, int objectiveId, int swarmSize, int maxIterations,
		...
		randomGenerator = new ExtendedPseudoRandomGenerator(new JavaRandomGenerator(JMetalRandom.getInstance().getSeed()));
	 ...
	 * @param evaluator
	 */
	public StandardPSO2011(DoubleProblem problem, int swarmSize, int maxIterations, int numberOfParticlesToInform,
	...
	public void initializeVelocity(List<DoubleSolution> swarm) {
		for (int i = 0; i < swarmSize; i++) {
			...
			for (int j = 0; j < problem.getNumberOfVariables(); j++) {
				speed[i][j] = (randomGenerator.nextDouble(particle.getLowerBound(j) - particle.getVariableValue(0),
		...
		for (int i = 0; i < swarmSize; i++) {
			...
			if (this.localBest[i] != this.neighborhoodBest[i]) {
				for (int var = 0; var < particle.getNumberOfVariables(); var++) {
					double G;
					G = particle.getVariableValue(var) + c * (localBest[i].getVariableValue(var)
							+ neighborhoodBest[i].getVariableValue(var) - 2 * particle.getVariableValue(var)) / 3.0;
			...
			double radius = 0;
			radius = distanceBetweenSolutionsInObjectiveSpace(gravityCenter, particle);

			double[] random = randomGenerator.randSphere(problem.getNumberOfVariables());
			...
			for (int var = 0; var < particle.getNumberOfVariables(); var++) {
				speed[i][var] = weight * speed[i][var] + randomParticle.getVariableValue(var)
	...
	public void perturbation(List<DoubleSolution> swarm) {
		/*
		 * MutationOperator<DoubleSolution> mutation = new
		 * PolynomialMutation(1.0/problem.getNumberOfVariables(), 20.0) ; for
		 * (DoubleSolution particle : swarm) { mutation.execute(particle) ; }
		 */
	}
	...
	private DoubleSolution getNeighborBest(int i) {
		...
		for (DoubleSolution solution : neighborhood.getNeighbors(getSwarm(), i)) {
			...
			if ((bestLocalBestSolution == null)
					|| (bestLocalBestSolution.getObjective(0) > localBest[solutionPositionInSwarm].getObjective(0))) {
